...
from .rl_utils import to_device
...
class ExperienceReplay:
    ...
    def push(self, transition: list) -> int:
        ...
        The replay buffer assums the state in the transition list
        is of shape B,H,C,H,W and that the last element in H is the latest
        observation in the history window.
        """

        with torch.no_grad():
            state = transition[0][:, -1:].to(self.device)
        # print(transition[0].shape, state.shape)
...
def get_valid_checkpoints(root, dopamine):
    ...
    if dopamine:
        ...
        replay_paths = [f for f in root.iterdir() if "replay_" in f.stem]
        ckpt_idxs = [int(item.stem.split("_")[-1]) for item in replay_paths]
        return ckpt_idxs
...
class OfflineExperienceReplay:
    ...
    def __init__(
        self, root, is_dopamine, batch_size=32, device=None, is_async=False, **kwargs
    ):
        self.__root = Path(root)
        self.__dopamine = is_dopamine
        self.__batch_size = batch_size
        self.__device = device
        self.__valid_ckpts = get_valid_checkpoints(self.__root, self.__dopamine)
        self.__replay_buffers = []  # where we store the ER buffers
        self.__is_async = is_async
        self.__replay_kwargs = kwargs

    def sample(self, idxs=None):
        er = random.choice(self.__replay_buffers)
        return er.sample(gods_idxs=idxs)

    def reload_N(self, N=1, ckpt_idxs=None, workers=1):
        ...
        with ThreadPoolExecutor(max_workers=workers) as tpe:
            replay_futures = [
                tpe.submit(self._load_one, idx, self.__dopamine) for idx in ckpt_idxs
            ]
    ...
    def _load_one(self, idx, dopamine=True):
        ...
        """
        replay_buffer = ExperienceReplay(**self.__replay_kwargs)
    ...
    def __str__(self):
        return "{}(root={}, N={}, dopamine={}, batch={})".format(
            self.__class__.__name__,
            str(self.__root),
            len(self.__valid_ckpts),
            self.__dopamine,
            self.__batch_size,
        )
