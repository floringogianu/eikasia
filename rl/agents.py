...
from rl.schedules import constant_schedule, get_schedule
...
class PolicyImprovement:
    ...
    def act(self, state):
        """ Epsilon Greedy Policy Improvement step."""
...
class C51PolicyEvaluation:
    ...
    def update_estimator(self):
        ...
        if self.clip_grad_norm:
            ...
            self.estimator.div_grad_by_rho(mode=self.div_grad_by_rho)
        self.optimizer.step()
...
def scaled_log_softmax(logits, tau=1.0, dim=-1):
...
def scaled_softmax(logits, tau=1.0, dim=-1):
...
def get_munchausen_dqn_loss(  # pylint: disable=bad-continuation
    batch,
    estimator,
    gamma,
    target_estimator,
    ...
    r""" Computes the Munchausen DQN loss.
    ...
    Args:
        ...
        alpha (float): Munchausen coefficient.
        loss_fn (torch.nn.Loss): Defaults to torch.nn.MSELoss. Custom loss
            function, eg.: torch.nn.SmoothL1Loss.

    Returns:
        MDQNLoss: A simple namespace containing the loss and its byproducts.
    """
    ...
    q_vals = estimator(states)
    qsa = q_vals.gather(1, actions)

    # compute soft log policy
    q_vals_target = target_estimator(states).detach()
    log_policy = scaled_log_softmax(q_vals_target, tau=tau)
    ...
    if next_states.nelement():
        next_q_vals = target_estimator(next_states).detach()
        next_log_policy = scaled_log_softmax(next_q_vals, tau=tau)
        next_policy = scaled_softmax(next_q_vals, tau=tau)
        next_soft_q = (next_q_vals - next_log_policy) * next_policy
        next_soft_qsa[mask] = next_soft_q.sum(dim=1, keepdim=True)

    munchausen_target = rewards + munchausen_term + gamma * next_soft_qsa

    loss = loss_fn(qsa, munchausen_target)

    return MDQNLoss(
        loss=loss,
        qsa=qsa,
        munchausen_target=munchausen_target,
        q_values=q_vals,
        q_targets=next_soft_q,
        entropy=torch.mean(-(next_policy * next_log_policy / tau).sum(1)),
    )
...
class MDQNPolicyEvaluation(DQNPolicyEvaluation):
    r""" Munchausen DQN.
    """
    ...
    def __call__(self, batch, cb=None, update=True, backward=True):
        r""" Performs a policy improvement step. """

        batch = to_device(batch, self.device)

        mdqn_loss = get_munchausen_dqn_loss(
            batch,
            self.estimator,
            self.gamma,
...
AGENTS = {
    ...
    "M-DQN": {
        "policy_improvement": PolicyImprovement,
        "policy_evaluation": MDQNPolicyEvaluation,
    },
}
