        ...
        **ignorekwargs,
        ...
        print(
            "Working with z of shape {} = {} dimensions.".format(
                self.z_shape, math.prod(self.z_shape)
            )
        )
...
def main():
    ...
    # summary
    ch, D = 3, 96
    encoder = _SDEncoder(in_channels=ch, resolution=D).to(device)
    decoder = _SDDecoder(in_channels=ch, resolution=D).to(device)

    # inference
    x = torch.randn((1, ch, D, D))
    z = encoder(x)
    x_ = decoder(z[:, :4, :, :])
